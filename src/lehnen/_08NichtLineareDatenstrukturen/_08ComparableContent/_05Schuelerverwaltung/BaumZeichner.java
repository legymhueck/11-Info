...
public class BaumZeichner extends Frame {
    ...
    BinarySearchTree<Schueler> _baum;
    BufferedImage img;
    Graphics g;
    int _dy;              //  Abstand der Baumebenen
    int _tiefe = 0;       //  Tiefe des Baumes

    public BaumZeichner(int frameWidth, int frameHeight, BinarySearchTree<Schueler> pBaum) {
        super("Baumzeichner");
        _baum = pBaum;
    ...
    private void zeichneInorder(int links, int rechts, int y, BinarySearchTree<Schueler> baum) {
        if (baum.isEmpty()) return;
        int mitte = (links + rechts) / 2;
        zeichneInorder(links, mitte, y + _dy, baum.getLeftTree());
        zeigeKnoten(links, rechts, y, baum.getContent().getNachname() + ", " + baum.getContent().getVorname());
    ...
    private void tief(BinarySearchTree<Schueler> baum, int t) {
        if (baum.isEmpty()) return;
    ...
    public int tiefe(BinarySearchTree<Schueler> baum) {
        _tiefe = 0;
        tief(baum, 1);
        return _tiefe;
    }
